...
As an example, we'll map census tracts in [Alameda County, California](https://en.wikipedia.org/wiki/Alameda_County,_California). Alameda County is socioeconomically diverse and exhibits a wide range of income/inequality relationships across its 361 census tracts.
...
Constructing a bivariate color scale that's (comparatively) easy for the viewer to understand and interpret can be a challenge. [Joshua Stevens's 2015 post](http://www.joshuastevens.net/cartography/make-a-bivariate-choropleth-map/), as well as [Grossenbacher's post](https://timogrossenbacher.ch/2019/04/bivariate-maps-with-ggplot2-and-sf/) speak to some of the challenges. Luckily, Stevens includes several options, and Christopher Prener has developed the `biscale` package ([GitHub](https://github.com/slu-openGIS/biscale) | [CRAN](https://cloud.r-project.org/web/packages/biscale/index.html)) to generate bivariate color scales and ggplot legends.

Unfortunately, for our purposes, we need to take a different route to get a tanslucent bivariate color scale. To orient ourselves when zooming in, we'll want to see street names and so forth on the base map. So we'll want transparency in our bivariate color scale, which will require transparency in each of two intersecting univariate scales, each of which is mapped to quantile bins of our income or inequality vairables. Ultimately, we'll need full-opacity color values for each cell in each univariate scale, plus the resulting values for each cell when the univariate scales are intersected with reduced opacity.
...
For the Gini coefficient palette, we'll use `colorQuantile()` instead of `colorBin()` and define the breaks at terciles (1/3-quantiles), just as in Grosenbacher's post. Here, we'll get the color hex values from the _blue_ univariate scale.
...
Unlike Leaflet's popups functionality, _labels_ don't accept HTML by default, so we need to do a little massaging to get a multiline tooltip-style label. Popups are certainly an alternative option, but I perfer the more automatic-feeling mouseover behavior of a label to the click-open-and-close behavior of a popup, especially when looking at small geographies like census tracts. The method to get a multiline label can be found [here](https://stackoverflow.com/questions/43144596/r-and-leaflet-how-to-arrange-label-text-across-multiple-lines). The geographic data in our main data frame messes with the function, so we need to drop the geometry column first.
...
Our basic functionality is now in pretty good shape, but we absolutely need a legend. Leaflet's built-in legends can't handle a bivariate color scale, so we'll build our own with `ggplot2` and add it with `leafem::addLogo()` (formerly in the `mapview` package).

We'll build our legend following Grosenbacher's methods with a few alterations. To match the values we see on the map, we need to eyedropper the 50% opacity hex values from our template. We'll give each value basic Cartestian coordinates, where x = income and y = inequality. We'll then create our legend using `geom_tile()`. Importantly using a transparent background in the ggplot theme, for reasons we'll get to shortly.
...
We'll want our legend to more or less follow the design of the existing zoom and layer control elements of our map. Perhaps the best method of doing so would be to replicate the CSS classes of those elements. But we can also get a close approximation entirely within R. We can use `grid` to create a white rounded rectangle with a low-opacity black/grey stroke, and stack our legend on top using `cowplot`. Saving to SVG format will preserve stroke opacity, and `leafem::addLogo()` will add the legend and background to our plot.

With the legend added, we have our final interactive bivariate choropleth.
